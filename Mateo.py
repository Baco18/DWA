...
def student_profile():
    ...
    if request.method == "POST": 
        name = request.form.get("ime")
        prisutnost = request.form.get("prisutnost")
        kolegij = request.form.get("kolegij")
        datum = request.form.get("datum")

        new_prisutnost = Prisutnost(prisutnost=prisutnost, prezime_studenta=name, naziv_kolegija=kolegij, datum=datum)  
        db.session.add(new_prisutnost)
        db.session.commit()
        return '<h1> dolaznost evidentirana.</h1>'
...
def prof_reg():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(new_user)
        db.session.commit()
        return redirect(url_for('prof_log'))
